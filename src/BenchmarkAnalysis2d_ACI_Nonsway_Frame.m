        ...
        function results = run_to_peak_proportional(obj,e)
            ...
            if strcmpi(obj.EIeff_type,'c') || strcmpi(obj.EIeff_type,'6.6.4.4.4c')
                ...
                for i = 1:num_points
                    ...
                    [Pi,~,exitflag,~] = fsolve(@(p)error_EI_type_c(obj,p,0,M2(i)),P_guess,opts);

                    if exitflag <= 0
                        if ~isinf(error_EI_type_c(obj,0.75*P_guess,0,M2(i)))
                            [Pi,~,exitflag,~] = fsolve(@(p)error_EI_type_c(obj,p,0,M2(i)),0.75*P_guess,opts);
                        end
                        
                        if exitflag <= 0
                            if ~isinf(error_EI_type_c(obj,0.5*P_guess,0,M2(i)))
                                [Pi,~,exitflag,~] = fsolve(@(p)error_EI_type_c(obj,p,0,M2(i)),0.5*P_guess,opts);
                            end
                            
                            if exitflag <= 0
                                if ~isinf(error_EI_type_c(obj,0.25*P_guess,0,M2(i)))
                                    [Pi,~,exitflag,~] = fsolve(@(p)error_EI_type_c(obj,p,0,M2(i)),0.25*P_guess,opts);
                                end
                                
                                if exitflag <= 0
                                    if ~isinf(error_EI_type_c(obj,0,0,M2(i)))
                                        [Pi,~,exitflag,output] = fsolve(@(p)error_EI_type_c(obj,p,0,M2(i)),0,opts);
                                    end
